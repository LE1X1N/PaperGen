  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-white via-gray-50 to-gray-100 text-gray-800">
      ...
      <header className="flex items-center justify-between h-14 px-6 bg-white border-b border-gray-200 shadow-sm sticky top-0 z-30">
        <div className="text-2xl font-extrabold text-indigo-600 select-none">Logo</div>
        <nav className="flex space-x-8 text-gray-600 text-lg font-medium">
          {navItems.map((item, idx) => (
            <button
              key={item}
              onClick={() => setSelectNav(idx)}
              className={`relative px-1 transition-colors duration-300 hover:text-indigo-600 ${
      ...
      {/* 主体部分：侧边栏 + 内容 */}
      <div className="flex flex-1 max-w-[1280px] mx-auto mt-6 mb-12 px-4 sm:px-6 lg:px-8 w-full">
        {/* 侧边栏 */}
        <aside className="w-60 bg-white rounded-lg shadow border border-gray-200 sticky top-20 h-[calc(100vh-5.5rem)] overflow-auto py-4">
          {sidebarItems.map(({ key, icon, label, children }) => (
            <div key={key} className="mb-6 last:mb-0">
              <div className="flex items-center px-6 mb-2 text-gray-700 font-semibold text-base select-none">
                  ...
                  return (
                    <li key={child} className="list-none">
                      <button
                        ...
                        className={`flex items-center w-full text-gray-600 hover:text-indigo-600 text-sm rounded-md py-1.5 px-6 transition-colors duration-200 ${
        ...
        <main className="flex-1 bg-white rounded-lg shadow border border-gray-200 p-8 min-h-[520px] select-text">
          ...
          <section className="text-gray-700 leading-relaxed space-y-4 text-base">
            <p>
